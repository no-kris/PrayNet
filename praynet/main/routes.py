...
def get_trending_prayers(days=7):
    ...
    return (
        PrayerRequest.query.filter(PrayerRequest.date_posted >= time_delta)
...
def home():
    ...
    if category == "all":
        ...
        query = PrayerRequest.query.filter_by(category=category).order_by(
            PrayerRequest.date_posted.desc()
        )
